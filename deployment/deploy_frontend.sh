...
gcloud run deploy "${SERVICE_NAME}" \
    ...
    --set-env-vars "USE_CLOUD_AGENT=true,VERTEX_AGENT_ENGINE_ID=${REASONING_ENGINE_ID},GOOGLE_CLOUD_PROJECT=${PROJECT_ID},GOOGLE_CLOUD_LOCATION=${LOCATION},GOOGLE_CLOUD_STORAGE_BUCKET=${GCS_BUCKET},GOOGLE_GENAI_USE_VERTEXAI=1,DB_USER=app_user,DB_PASSWORD=ediscovery_pass_123,DB_NAME=ediscovery,DB_HOST=/cloudsql/${PROJECT_ID}:${LOCATION}:ediscovery-db,DB_PORT=5432" \
    --memory=2Gi \
